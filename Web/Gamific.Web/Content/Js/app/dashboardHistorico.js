...
function refreshDropDownEpisodes(state, currentId) {
    $.ajax({
        url: "/public/dashboardHistorico/buscarEpisodios",
        async: true,
        type: "GET",
        data:
        {
...
function refreshDropDownTeams(episodeId, currentId) {
    $.ajax({
        url: "/public/dashboardHistorico/buscarEquipes",
        async: true,
        type: "GET",
        data:
        {
...
function refreshDropDownWorkers(teamId, currentId) {
    $.ajax({
        url: "/public/dashboardHistorico/buscarJogadores",
        async: true,
        type: "GET",
        data:
        {
...
function refreshDropDownItens() {
    $.ajax({
        url: "/public/dashboardHistorico/buscarItens",
        async: true,
        type: "GET",
        success: function (data) {
...
function loadMetricList() {
    ...
    $.ajax({
        url: "/public/dashboardHistorico/buscarMetricasPorCampanha/" + $('#dropDownEpisodes').val(),
        async: true,
        type: "GET",
        success: function (data) {
...
function refreshCardResults(episodeId, teamId, playerId, itemId) {
    $('#div-cards').empty();
    $.ajax({
        url: "/public/dashboardHistorico/buscarResultados",
        async: true,
        type: "GET",
        data:
        {
            ...
            for (var i in cardResults) {
                ...
                if (cardResults[i].metricName == "Check-In") {
                    card += "<div class='card-container col-lg-3 col-md-3 col-sm-6'>"
                            ...
                            + "</div>"
                            + "</div>"
                            + "<div class='back'><a href='/public/dashboardHistorico/detalhesCheckin/" + episodeId + "/" + cardResults[i].metricId + "/" + teamId + "/" + playerId + "'><i class='fa " + icon + " fa-4x'></i><span>Detalhes</span></a>"
                            + "</div>"
                            + "</div>"
                            + "</div>";
                } else {
                    card += "<div class='card-container col-lg-3 col-md-3 col-sm-6'>"
                            ...
                            + "</div>"
                            + "</div>"
                            + "<div class='back'><a href='/public/dashboardHistorico/detalhes/" + episodeId + "/" + cardResults[i].metricId + "/" + teamId + "/" + playerId + "'><i class='fa " + icon + " fa-4x'></i><span>Detalhes</span></a>"
                            + "</div>"
                            + "</div>"
                            + "</div>";
                }
...
function LoadMetricResultsDataTable() {
    table = $('#MetricResultsDataTable').dataTable({
        "serverSide": true,
        "ajax": "/public/dashboardHistorico/resultadosMetrica/" + $('#MetricId').val() + "/" + $('#EpisodeId').val() + "/" + $('#TeamId').val() + "/" + $('#PlayerId').val(),
        "processing": true,
        "scrollY": "300px",
        "scrollCollapse": true,
        "deferRender": true,
        ...
        "columnDefs": [
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    var value = row[3];
                    var name = row[1];
                    var links = "<a class='fa fa-pencil' onclick='showEntityModal(this); return false;' href='/public/dashboardHistorico/editar/" + data + "' title='Editar resultado.'> </a> &nbsp; <a class='fa fa-remove' href='#' onclick='removeClickResult(\"" + data + "\",\"" + value + "\",\"" + name + "\")' title='Remover resultado.'> </a>";
                    return links;
                }
            }
        ],
...
function Remove(resultId) {
    $.ajax({
        url: "/public/dashboardHistorico/remover/" + resultId,
        async: true,
        type: "POST",
        success: function () {
    ...
    var dialog = BootstrapDialog.show({
        ...
        buttons: [{
            ...
            action: function (dialog) {
                $.ajax({
                    url: "/public/dashboardHistorico/remover/" + data,
                    async: true,
                    type: "POST",
                    success: function () {
...
function initializeChart() {
    ...
    if (campaignId && campaignId.length > 0) {
        ...
        $.ajax({
            url: "/public/dashboardHistorico/loadChart",
            async: false,
...
function loadMorris(type) {
    ...
    if (type == 1) {
        $.ajax({
            url: "/public/dashboardHistorico/loadMorrisByEpisode/" + metricId + "/" + episodeId,
            async: false,
            type: "GET",
            success: function (data) {
                Morris.Donut({
    ...
    if (type == 2) {
        $.ajax({
            url: "/public/dashboardHistorico/loadMorrisByTeam/" + metricId + "/" + teamId,
            async: false,
            type: "GET",
            success: function (data) {
                Morris.Donut({
    ...
    if (type == 3) {
        $.ajax({
            url: "/public/dashboardHistorico/loadMorrisByRun/" + metricId + "/" + runId + "/" + teamId,
            async: false,
            type: "GET",
            success: function (data) {
                Morris.Donut({
...
function loadBarChart() {
    ...
    $.ajax({
        url: "/public/dashboardHistorico/loadBarChart",

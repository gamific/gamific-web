        ...
        public void Insert(TUser user, string roleName)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var idObject = MySqlHelper.ExecuteScalar(conn, CommandType.Text,
                    @"SELECT Id FROM Account_Role WHERE Name=@RoleName", parameters);
                ...
                if (!string.IsNullOrEmpty(roleId))
                {
                    ...
                    using (var conn1 = new MySqlConnection(_connectionString))
                    {
                        ...
                        MySqlHelper.ExecuteNonQuery(conn1, @"Insert into Account_UserRole(UserId,RoleId) VALUES(@Id,@RoleId)", parameters1);
                    }
                }
            }
        }
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                if (!string.IsNullOrEmpty(roleId))
                {
                    ...
                    using (var conn1 = new MySqlConnection(_connectionString))
                    {
                        ...
                        MySqlHelper.ExecuteNonQuery(conn1, @"Delete FROM Account_UserRole WHERE UserId=@Id AND RoleId=@RoleId", parameters1);
                    }
                }
            }
        }
        ...
        public List<String> PopulateRoles(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                    @"SELECT RoleId FROM Account_UserRole Where UserId=@Id", parameters);
                while (reader.Read())
                {
            ...
            foreach (var roleId in roleIds)
            {
                    ...
                    var parameters = new Dictionary<string, object>
                {
                    ...
                    var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                        @"SELECT Name FROM Account_Role Where Id=@Id", parameters);
                    while (reader.Read())
                    {

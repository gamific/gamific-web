    ...
    public class UserLoginRepository
    {
        ...
        public void Insert(IdentityUser user, UserLoginInfo login)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn, @"INSERT INTO Account_UserLogin(UserId,LoginProvider,ProviderKey) VALUES(@Id,@LoginProvider,@ProviderKey)", parameters);
            }
        }
        ...
        public void Delete(IdentityUser user, UserLoginInfo login)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn, @"DELETE FROM Account_UserLogin WHERE UserId = @Id AND LoginProvider = @LoginProvider AND ProviderKey = @ProviderKey", parameters);
            }
        }
        ...
        public string GetByUserLoginInfo(UserLoginInfo login)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var userIdObject = MySqlHelper.ExecuteScalar(conn, CommandType.Text,
                    @"SELECT UserId FROM Account_UserLogin WHERE LoginProvider = @LoginProvider AND ProviderKey = @ProviderKey", parameters);
        ...
        public List<UserLoginInfo> PopulateLogins(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                   @"SELECT LoginProvider,ProviderKey FROM Account_UserLogin Where UserId = @Id", parameters);
                while (reader.Read())
                {
